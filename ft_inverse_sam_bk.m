...
for i=1:size(sourcemodel.pos,1)
    ...
    if hasfilter
        ...
        if size(lf,1)>306
            ...
            [eig_sort, B_inx] = sort(eig, 'descend');
            B = B(:,B_inx);
            for k=i:numel(eig_sort)
                Jota(k)=(sum(eig_sort(1:k)))/ (sum(eig_sort))*100;
            end
            Jota=find(Jota>80);
            Jota=Jota(1);
            Bs=B(:,1:Jota);
            ...
            As=lf'*Ps*lf;
            As=diag(As);
            opt_vox=opt_vox_or.*As;
            gain       = lf * opt_vox; % MSP On
        else
            %         compute the spatial filter for the optimal source orientation
            gain        = lf * opt_vox_or; % MSP Off
        end
        trgain_invC = gain' * inv_cov;
        SAMweights  = trgain_invC / (trgain_invC * gain);
        SAMweightsnorm = SAMweights / sqrt(noise^2*(SAMweights*SAMweights'));
    ...
    if ~isempty(dat)
        ...
        estimate.momnorm{i} = SAMweightsnorm * dat; %bk
    end
